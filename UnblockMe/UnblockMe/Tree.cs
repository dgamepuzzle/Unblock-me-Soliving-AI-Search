        ...
        public List<Tree> createNewTreeWithAvailableAction () {
            ...
            for (int i = 0; i < this.carlist.Count; i++) {
                if (this.checkStepAvailable (carlist[i]).Count != 0) {
                    // Console.WriteLine ("check: " + this.checkStepAvailable (carlist[i]).Count);
                    foreach (int item in this.checkStepAvailable (carlist[i])) {
                        ...
                        int tempAction = setAction (this.carlist[i].id, item);

                        // Console.WriteLine ("Fucking TempPath : ");
                        // tempPath.ForEach (Console.WriteLine);
                        // Console.WriteLine ("**********");

                        temp.action = tempAction;
                        ...
                        if (temp.action != oppositAction) {
                            ...
                            tempPath.AddRange (this.path);
                            tempPath.Add (tempAction);
                            temp.path = tempPath;
                            treeList.Add (temp);
                        }
                    }
